...
sto=Storage(0.3,0,1.0,1,Array(Float64,(45,45,3)),x->0,15,0,0,0,0.0,Array(Float64,1,1,1),Array(Complex,1,1,1),x->0)
...
setpoints()
function mag(nitude::Float64,color::Array{Float64,1},size::Float64=0.5,right::Bool=true)
	...
	glBegin(GL_POLYGON)
		...
		if right==true
			...
			glVertex(0.0,size,0.0)
		end
		glColor3f(color[1]*1.5,color[2]*1.5,color[3]*1.5)
		glVertex(size*0.5,size*0.5,nitude)
	glEnd()
	glColor3f(color[1],color[2],color[3])
	glBegin(GL_POLYGON)
		glVertex(0.0,0.0,0.0)
		glVertex(size,size,0.0)
		glColor3f(color[1]*1.5,color[2]*1.5,color[3]*1.5)
		glVertex(size*0.5,size*0.5,nitude)
	glEnd()
	glColor3f(color[1],color[2],color[3])
			...
			for y=1:l
				for z=1:l
					glPushMatrix()
						...
						#glRotate(180.0,1.0,0.0,1.0)
						mag(imag(sto.rval[x,y,z]),[0,0,0.66],q/1.5,false)
					glPopMatrix()
				end
			end
		end
